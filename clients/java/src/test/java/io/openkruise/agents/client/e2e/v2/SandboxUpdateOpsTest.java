...
public class SandboxUpdateOpsTest extends BaseE2eTest {
    ...
    public void testSandboxUpdateOpsBatchUpgrade() throws Exception {
        ...
        ObjectMeta sbxMeta = new ObjectMeta();
        sbxMeta.setName(sandboxName);
        sbxMeta.setNamespace(NAMESPACE);
        Map<String, String> sbxLabels = new HashMap<>();
        sbxLabels.put("e2e-ops-batch", "upgrade-test");
        sbxMeta.setLabels(sbxLabels);
        sandbox.setMetadata(sbxMeta);
        ...
        ObjectMeta opsMeta = new ObjectMeta();
        opsMeta.setName(opsName);
        opsMeta.setNamespace(NAMESPACE);
        ops.setMetadata(opsMeta);
        ...
        assertNull(client.resources(SandboxUpdateOps.class).inNamespace(NAMESPACE).withName(opsName).get());
    }
}
