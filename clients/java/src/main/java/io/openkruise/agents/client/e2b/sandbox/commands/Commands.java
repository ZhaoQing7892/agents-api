...
public class Commands {
    ...
    public Commands(ManagedChannel channel, ConnectionConfig config) {
    ...
    public List<ProcessInfo> list() {
        ...
        try {
            ListResponse response = blockingStub
                .withDeadlineAfter(config.getRequestTimeoutMs(), TimeUnit.MILLISECONDS)
                .list(request);

            return response.getProcessesList().stream()
    ...
    public CommandHandle runBackground(String cmd) {
        return runBackground(cmd, new RunOptions());
    }
    ...
    public void sendStdin(long pid, String data) {
        validatePid(pid);
        if (data == null) {
        ...
        try {
            blockingStub.withDeadlineAfter(config.getRequestTimeoutMs(), TimeUnit.MILLISECONDS)
                .sendInput(request);
        } catch (StatusRuntimeException e) {
            throw RpcUtils.handleRpcException(e);
        }
    ...
    public boolean kill(long pid) {
        return kill(pid, Signal.SIGNAL_SIGKILL);
    }

    public boolean kill(long pid, Signal signal) {
        validatePid(pid);

        SendSignalRequest request = SendSignalRequest.newBuilder()
            .setProcess(ProcessSelector.newBuilder().setPid((int)pid).build())
            .setSignal(signal != null ? signal : Signal.SIGNAL_SIGTERM)
            .build();
    ...
    public CommandHandle connect(long pid) {
        validatePid(pid);
