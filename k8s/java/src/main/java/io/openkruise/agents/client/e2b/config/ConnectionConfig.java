...
public class ConnectionConfig {
    ...
    public String getSandboxHost(String sandboxId, String sandboxDomain) {
        if (debug) {
            return sandboxDomain != null ? sandboxDomain : domain;
        }
    ...
    public ManagedChannel createChannel(String sandboxId, String token, String sandboxDomain) {
        ...
        ManagedChannelBuilder<?> builder = NettyChannelBuilder.forTarget(target);
    ...
    public Request buildHttpRequest(String serviceName, String methodName, MessageOrBuilder params, String sandboxId) {
        try {
            String jsonPayload = JsonFormat.printer().print(params);
            RequestBody requestBody = RequestBody.create(jsonPayload, MediaType.parse("application/json"));

            return new Request.Builder()
                .url(getHttpUrlBuilder(sandboxId).addPathSegment(serviceName).addPathSegment(methodName).build())
                .post(requestBody)
                .headers(getHttpHeaders(sandboxId))
                .build();
        } catch (Exception e) {
            throw new RuntimeException(String.format("build http request Failed to %s", methodName), e);
        }
    }

    private HttpUrl.Builder getHttpUrlBuilder(String sandboxId) {
        String protocol = debug ? "http" : "https";
        String host = debug ? domain : String.format("%d-%s.%s", ConnectionConfig.ENVD_PORT,
            sandboxId, domain);
        if (debug) {
            return new HttpUrl.Builder()
                .scheme(protocol)
                .host(host)
                .addPathSegments("kruise")
                .addPathSegments(sandboxId)
                .addPathSegments(String.valueOf(ConnectionConfig.ENVD_PORT));
        } else {
            return new HttpUrl.Builder()
                .scheme(protocol)
                .host(host);
    ...
    private Headers getHttpHeaders(String sandboxId) {
        Headers.Builder builder = new Headers.Builder()
            .add("X-API-Key", apiKey)
            .add("Authorization", "Basic cm9vdDo=")
            .add("E2b-Sandbox-Id", sandboxId)
            .add("E2b-Sandbox-Port", String.valueOf(ConnectionConfig.ENVD_PORT));
