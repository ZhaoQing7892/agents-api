...
run_go_test() {
  ...
  cd "${PROJECT_ROOT}"
  go test -v -timeout "${TIMEOUT}s" -tags=e2e ./test/e2e/...
...
run_python_test() {
  ...
  pip install -r requirements-test.txt -q 2>/dev/null || pip3 install -r requirements-test.txt -q 2>/dev/null
  pip install -e . -q 2>/dev/null || pip3 install -e . -q 2>/dev/null
  python -m pytest e2e/sdk_test.py -v --timeout="${TIMEOUT}"
