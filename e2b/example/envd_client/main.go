// Example: connect directly to an existing sandbox's envd service without
// going through the sandbox-management REST API.
//
// Usage:
//
//	go run ./e2b/example/envd_client
//
// Replace `sandboxID` and connection parameters with your own values.
package main
...
const (
	// sandboxID must point to an already-running sandbox. This client never
	// creates or destroys sandboxes; it only operates on the envd service of
	// an existing one.
...
func main() {
	...
	c := client.New(sandboxID,
		client.WithDomain(domain),
		client.WithRuntimeToken(runtimeToken),
...
func demonstrateCommandOperations(ctx context.Context, commands *client.Commands) {
	...
	if result, err := commands.Run(ctx, "echo $TEST_VAR && pwd", client.RunOpts{
		Cwd:  "/tmp",
...
func listProcesses(ctx context.Context, commands *client.Commands) {
...
func isProcessRunning(ctx context.Context, commands *client.Commands, pid uint32) bool {
	processes, err := commands.List(ctx)
	if err != nil {
		return false
	}
...
func demonstrateFileOperations(ctx context.Context, files *client.Filesystem) {
...
func listEntries(ctx context.Context, files *client.Filesystem, path string) {
