...
func demonstrateCommandOperations(ctx context.Context, commands *envdclient.Commands) {
	...
	if result, err := commands.Run(ctx, "echo $TEST_VAR && pwd", envdclient.RunOpts{
		Cwd:  "/tmp",
...
func listProcesses(ctx context.Context, commands *envdclient.Commands) {
...
func isProcessRunning(ctx context.Context, commands *envdclient.Commands, pid uint32) bool {
	processes, err := commands.List(ctx)
	if err != nil {
		return false
	}
...
func demonstrateFileOperations(ctx context.Context, files *envdclient.Filesystem) {
...
func listEntries(ctx context.Context, files *envdclient.Filesystem, path string) {
