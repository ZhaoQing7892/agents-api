...
// This allows embedding connection configuration (API key, domain, etc.)
// directly into Create/Connect calls.
...
// Sandbox is the all-in-one entry point that combines the management API
// (lifecycle: Create/Connect/Pause/Kill/...) with the in-sandbox envd client
// (data plane: Files / Commands / SandboxID via the embedded
// envdclient.Client).
//
// If you only need to operate an existing sandbox without going through the
// management API, use envdclient.New directly.
type Sandbox struct {
	// Embedded envd client exposes Commands, Files, SandboxID, etc.
	*envdclient.Client
...
//
// The template defaults to "code-interpreter" if empty. Connection configuration is
// read from environment variables by default and can be overridden via
// WithConfig(...) option.
...
// Connect connects to an existing sandbox by its ID.
//
// If the sandbox is paused, it will be resumed. Connection configuration is
// read from environment variables by default and can be overridden via
// WithConfig(...) option.
...
// newSandbox initializes a Sandbox by composing an envdclient.Client (data
// plane) with the management-API SandboxApi (control plane).
func newSandbox(config *ConnectionConfig, api *SandboxApi, sandboxID, templateID, envdVersion string) *Sandbox {
	client := envdclient.NewWithConfig(sandboxID, config.toEnvdConfig(sandboxID))
